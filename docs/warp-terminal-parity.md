...
| Inline images | SIXEL and iTerm2 inline image (IIP) output rendered in the terminal grid | Image sequences discarded, printing garbage | `@xterm/addon-image` loaded before `open()` so its parser hooks cover the first byte of output. Budgets are tightened from the addon defaults because they are per terminal and this app keeps one alive per opened session: 8 MB image store, 2048x2048 decode limit, 4 MB per sequence. The decode limit is deliberately below the store limit, since the storage evictor admits an over-sized image once it has nothing left to evict. The addon enables the CSI 14/16/18 t size reports so programs can size their output; those replies travel on the allowlisted `terminalResponse` transport, not the user-input path. Verified end to end through a live PTY: a SIXEL band and a scaled IIP PNG both decode into an `xterm-image-layer` canvas. |
| Renderer | GPU-accelerated grid rendering with a safe fallback | xterm DOM renderer only | `@xterm/addon-webgl` with automatic fallback: a lost context disposes the addon, returns the terminal to the DOM renderer, and retries GPU once before staying on DOM for the session. Activation failure (no WebGL2) falls through the same path. Contexts follow what is on screen rather than creation order: a terminal claims one when a pane attaches and gives it back when its last pane detaches, and at most 4 are held at once, well under Chromium's per-renderer ceiling. |
...
63. Persistence schema versions are an authority boundary, not decorative
    metadata. Application state accepts the original unversioned shape plus
    explicit v1, v2, and current v3; validated legacy data migrates atomically
    to v3 while preserving the compatible v2 generation as its rollback
    backup. Scrollback and terminal blocks retain their current v2 auxiliary
    envelopes. Any other explicit version is unsupported, never coerced, and
    never rewritten during load. A compatible backup may be used in memory,
    but the incompatible current path becomes read-only so periodic autosave
    cannot perform an implicit downgrade. Only removing that current file or
    running a build that supports its version releases the guard. Corrupt JSON
    and semantically invalid current data continue to use the separate
    validated-backup recovery path.
...
- Broader unsafe escape filtering. OSC 8 hyperlink activation and
  embedded-browser navigation are HTTP(S)-only at the Electron authority
  boundary; OSC 52 clipboard policy and history isolation are implemented.
  Image protocols are now implemented (see the capability matrix).
...
- Persistence version skew: deterministic state cases migrate unversioned, v1,
  and v2 application state to v3 while preserving the compatible v2 rollback
  backup; scrollback and block cases retain v2 envelopes, including migration
  from the original legacy text scrollback format. Current versions round trip
  unchanged. Explicit future and non-numeric versions are rejected, compatible
  backups restore without rewriting the unsupported current file, and simulated
  periodic saves cannot overwrite either future state or its last compatible
  backup. Explicit current file removal releases the state write guard. The
  source feature suite covers the v3 title-provenance migration and retained-v2
  auxiliary envelopes.
